...
- 修复勾选“显示画面”后任务一直停在“连接中”：任务 Sidecar 实际已进入 `connect-state-3`，但同步 Tauri 命令在主线程内创建 WebView 窗口会形成自等待，导致前端的窗口命令不返回。将 `open_test_task_window` 和 `close_test_task_window` 改为异步命令，并增加回归检查，避免窗口生命周期命令再次变回同步。
- 完整验证通过：前端 105 项、Rust 45 项、生产构建和 debug Tauri 构建均成功；用重编译后的真实应用勾选知识库“显示画面”后，任务进入绿色“已连接”、独立画面窗口显示实时虚机画面，停止任务后窗口同步消失。
- 测试画面连接后复用正常云桌面的 `desktopViewport` 分辨率计算，以及 `screen-change`、`invalidate-full-image`、`vdagent-send` 输入序列；画面窗口尺寸变化时也会重新同步分辨率。
- 新增薄层 `test_task_send_input` Tauri 命令，将输入直接转发给该任务独占的 `SidecarManager`，没有增加新的 Sidecar 协议或复制连接实现。
- 新增分辨率同步回归检查；完整前端 106 项、Rust 45 项、生产构建和 debug Tauri 构建通过。真实虚机实测显示窗口同步为 1680×1080，首帧尺寸与同步结果一致，停止任务后画面窗口消失。
- 修复测试台连接状态变化时列表列宽跳动：表格改用 CSS 固定列布局，避免“自动重连/停止”和状态文本长度变化重新分配各列；新增回归检查，完整前端 107 项测试与生产构建通过。
- 后续真实界面复测发现，仅固定列宽仍会让“自动重连/停止”和“空闲/已连接”控件自身宽度变化，视觉上继续跳动；现为操作按钮和状态文本分别保留固定宽度，并在重新编译的 debug 应用里完成“空闲 → 已连接 → 停止”回归，动态元素占位保持一致。

## 结论

测试台现在同时支持一次性保持连接和定时重连压力测试；每项任务有独立进程和独立资源趋势，停止任务会同步回收该进程。连接持续时间只要是正数即可启动，不再因小于 100 毫秒而静默禁用按钮。测试任务在主进程存活期间不依赖测试台页面生命周期，切换页面后仍会继续运行并在返回时恢复显示。资源曲线具备时间和数值坐标，可切换 1、5、15 分钟或 1 小时观察窗口并读取窗口平均值。勾选“显示画面”后，测试会话连接期间显示独立实时画面窗口，连接成功及窗口缩放后复用正常云桌面逻辑同步虚机分辨率，断开时窗口同步关闭。

## 待办

- [x] 在有真实登录态和可连接虚机的 Tauri 运行环境中人工确认缩略图、独立画面窗口及停止回收效果。
