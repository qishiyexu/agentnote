...
- 选择分组后默认常显分组名称、`arg1`、`arg2` 输入框；内容未变化时保存按钮禁用，修改后启用，不再提供单独的“编辑”和“取消”按钮。
- 三个分组输入框使用可见 label，复用现有中英文分组字段文案。
...
- 代码提交：`fa83162 feat: 为 SSH 快捷命令添加分组参数`、`505cd57 fix: 保留删除分组后的快捷命令语义`、`66e1d54 refactor: 简化 SSH 快捷命令分组编辑`、`6d1a55f ui: 为 SSH 分组参数输入框添加标签`。
