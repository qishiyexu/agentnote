...
服务器监控只在监控面板可见时工作，并复用已有 SSH 登录态，不会每 5 秒重新登录。磁盘数据当前以服务器根文件系统 `/` 为准；SFTP Tab 在文件列表下方显示活动传输，上传与下载进度均由后端实际完成的分块读写字节驱动，同一 SFTP Tab 的多个下载任务串行排队执行，任务入队时固定使用当时选择的下载目录。
...
- `git diff --check`：通过。
- 已推送 `thor_terminal/main`：`7141a0a`。
