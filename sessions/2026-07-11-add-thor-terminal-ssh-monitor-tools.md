...
- SFTP 文件栏增加“下载目录”按钮，复用现有文件夹选择器预设后续任务的本地目录；未预设时仍在首次下载时选择。提交：`7141a0a feat: 增加 SFTP 下载目录选择按钮`。
- 点击已有 SSH Tab，或从 SSH 配置按钮打开新 Tab、切入共享窗口中的已有 Tab 时，统一触发 xterm 焦点请求；激活后可直接键盘输入。提交：`04f47dd fix: 切换 SSH 标签时聚焦终端`。

## 结论

服务器监控只在监控面板可见时工作，并复用已有 SSH 登录态，不会每 5 秒重新登录。磁盘数据当前以服务器根文件系统 `/` 为准；SFTP Tab 在文件列表下方显示活动传输，上传与下载进度均由后端实际完成的分块读写字节驱动，同一 SFTP Tab 的多个下载任务串行排队执行，任务入队时固定使用当时选择的下载目录。SSH Tab 的打开与选择统一复用终端焦点请求，切换后无需再次点击终端即可输入。
...
- `git diff --check`：通过。
- 已推送 `thor_terminal/main`：`04f47dd`。
